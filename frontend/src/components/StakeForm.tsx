...
export const StakeForm: FC = () => {
  ...
  useEffect(() => {
    ...
    const expectedSlpSol = calculateSlpForSol(lamports);
    setOutputPreview((Number(expectedSlpSol) / LAMPORTS_PER_SOL).toFixed(6));
  }, [amount, poolConfig, calculateSlpForSol]);
  ...
  const handleStake = async () => {
    ...
    await initiateStake(numAmount, slippageBps);
  };

  const handleReveal = async () => {
    await executeStakeReveal();
  };

  const isValidAmount = () => {
    const numAmount = parseFloat(amount);
    return numAmount > 0 && numAmount * LAMPORTS_PER_SOL >= MIN_AMOUNT_LAMPORTS && numAmount <= solBalance;
  };

  // Show reveal button if user has a stake commitment
  const showRevealButton = hasCommitment && commitment?.isStake ? true : false;
  const canExecuteReveal = showRevealButton && commitment && canReveal(commitment, MIN_DELAY_SECONDS);

  if (!connected) {
    return (
      <div className="text-center py-12">
        <h3 className="text-xl font-semibold text-zinc-300 mb-2">Connect Wallet to Stake</h3>
        <p className="text-zinc-500">Connect your wallet to start staking SOL for secuSOL.</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-2">
        ...
        <div className="relative">
          <input
            ...
            className="w-full bg-zinc-800/50 border border-zinc-700/50 rounded-xl px-4 py-4 text-2xl font-semibold text-white placeholder-zinc-600 focus:outline-none focus:border-violet-500/50 focus:ring-1 focus:ring-violet-500/50 disabled:opacity-50"
          />
          <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-2">
            <button
              onClick={handleMaxClick}
              disabled={showRevealButton}
              className="px-2 py-1 text-xs font-medium text-violet-400 bg-violet-500/20 rounded hover:bg-violet-500/30 transition-colors disabled:opacity-50"
            >
              MAX
            </button>
            <span className="text-zinc-400 font-medium">SOL</span>
          </div>
        </div>
        {parseFloat(amount) > 0 && parseFloat(amount) * LAMPORTS_PER_SOL < MIN_AMOUNT_LAMPORTS && (
          <p className="text-red-400 text-sm">Minimum amount is 0.01 SOL</p>
        )}
      </div>
        ...
        <div className="bg-zinc-800/30 rounded-xl p-4 space-y-3">
          <div className="flex justify-between items-center">
            ...
            <span className="text-lg font-semibold text-emerald-400">
              ~{outputPreview} secuSOL
            </span>
          </div>
          <div className="flex justify-between items-center">
            <span className="text-zinc-400 text-sm">Exchange Rate</span>
            <span className="text-zinc-300 text-sm">1 SOL = {(1 / exchangeRate).toFixed(6)} secuSOL</span>
          </div>
        </div>
      )}
      ...
      {showRevealButton ? (
        <button
          ...
          `}
        >
          {canExecuteReveal ? "Execute Stake" : "Waiting for Delay..."}
        </button>
      ) : (
        <button
          onClick={handleStake}
